...
# فرض می‌شود این فایل‌ها در پوشه utils قرار دارند
...
def _run_horoscope_sync(data):
    ...
    # داده‌ها قبلاً توسط normalize_digits به ASCII تبدیل شده‌اند.
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    **این تابع برای نمایش صحیح کلیدها اصلاح شده است.**
    """
    keyboard = [
    ...
    # --- منطق اصلاح شده برای نمایش کلیدها ---
    if update.callback_query:
        # اگر از طریق یک دکمه (مثلاً /cancel یا دکمه‌های منو) فراخوانی شده، پیام را ویرایش کن.
        query = update.callback_query
        await query.answer()
        await query.message.edit_text(message_text, reply_markup=reply_markup)
        
    elif update.message:
        # برای فرمان /start، پیام جدید با کلیدها ارسال می‌شود.
        await update.message.reply_text(message_text, reply_markup=reply_markup)
        
    else:
        # در سایر حالت‌ها، مثل حالت‌های غیرمعمول، پیام جدید ارسال می‌شود.
...
async def get_city(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # **اجرای Geocoding در یک ترد مجزا (رفع مشکل Blocking)**
        ...
        if location:
            ...
            # **اجرای تولید هوروسکوپ در یک ترد مجزا (رفع مشکل Blocking)**
            text, img_path = await asyncio.to_thread(_run_horoscope_sync, data)
            
            # گام ۳: پاسخ به کاربر
            if img_path and os.path.exists(img_path):
                 # اگر فایل تصویر وجود دارد، آن را ارسال کن
